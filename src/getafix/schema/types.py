...
class TypeCode(enum.StrEnum):
    ...
    "Credit Note"
    T_SelfBilledCorrectiveInvoice = "471"
    "Invoice"
    T_FactoredCorrectiveInvoice = "472"
    "Invoice"
    T_SelfBilledFactoredCorrectiveInvoice = "473"
    "Invoice"
    T_SelfPrepaymentInvoice = "500"
    "Invoice"
    T_SelfBilledFactoredInvoice = "501"
    "Invoice"
    T_SelfBilledFactoredCreditNote = "502"
    "Credit Note"
    T_PrepaymentCreditNote = "503"
    "Credit Note"
    T_SelfBilledDebitNote = "527"
    "Invoice"
    T_ForwardersCreditNote = "532"
    "Credit Note"
    T_ForwardersInvoiceDiscrepancyReport = "553"
    "Invoice"
...
class UNTDID1001TypeCode(enum.StrEnum):
    ...
    """'Price/sales catalogue response' → tender or lot reference (BT-17)."""
    T_InvoicingDataSheet = "130"
    """'Invoicing data sheet' → invoiced-object identifier (BT-18)."""
    T_ReferencePaper = "916"
    """'Reference paper' → supporting-document reference (BT-122)."""
...
class CategoryCode(enum.StrEnum):
    ...
    T_S = "S"
    "Standard rate (BR-S-*); rate must be > 0"
    T_Z = "Z"
    "Zero rated (BR-Z-*); rate = 0; forbids exemption reason"
    T_E = "E"
    "Exempt from VAT (BR-E-*); rate = 0; requires exemption reason"
    T_AE = "AE"
    "Reverse charge (BR-AE-*); rate = 0; requires Buyer + Seller VAT IDs"
    T_K = "K"
    "Intra-community supply (BR-IC-*); also requires BT-72 or BG-14, plus BT-80"
    T_G = "G"
    "Export outside EU (BR-G-*); rate = 0; Seller VAT or tax-rep VAT (NOT BT-32)"
    T_O = "O"
    "Not subject to VAT (BR-O-*); rate forbidden, exclusive (BR-O-11..14)"
    T_L = "L"
    "IGIC — Canary Islands (BR-AF-*); rate ≥ 0"
    T_M = "M"
    "IPSI — Ceuta / Melilla (BR-AG-*); rate ≥ 0"
