...
| `check_verbatim.py` | Fails if any schema class / field docstring shares a run of 6+ consecutive words with the workbook's English text columns (descriptions, usage notes, rule prose) — copied spec wording is a licensing risk; docstrings must paraphrase. Official term names cited with their BT/BG id, code-list entries and legal citations are allow-listed in `ALLOWED_RUNS` (keyed by the normalised phrase, so copying *more* prose around an allow-listed phrase still fails). |
