...
| BT-X / BG-X | Name | Target | Card. | Notes |
|---|---|---|---|---|
| BT-X-204 ✓ | ``InvoiceIssuerReference`` | ``TradeSettlement.invoice_issuer_reference: str \| None`` | 0..1 | Landed. XSD position: between ``InvoiceCurrencyCode`` and ``InvoicerTradeParty`` (**not** first child — and the BT id is **BT-X-204**, not BT-X-217 as the original plan said; BT-X-217 is an Invoicer address line). Exercised by ``EXTENDED_synth_settlement_parties.xml``. |
| BG-X-33 ✓ | ``InvoicerTradeParty`` | ``InvoicerTradeParty`` in ``schema/party.py``; field ``TradeSettlement.invoicer`` | 0..1 | Landed. |
| BG-X-36 ✓ | ``InvoiceeTradeParty`` | ``InvoiceeTradeParty`` in ``schema/party.py``; field ``TradeSettlement.invoicee`` | 0..1 | Landed. Distinct from ``Buyer`` for factoring / payer-pays-third-party flows. (The mustangproject base sample already carried one — every synth sample now round-trips it.) |
| BG-X-73 ✓ | ``PayerTradeParty`` | ``PayerTradeParty`` in ``schema/party.py``; field ``TradeSettlement.payer`` | 0..1 | Landed. XSD position: between ``PayeeTradeParty`` and ``TaxApplicableTradeCurrencyExchange``. |
...
| BG-X-42 | ``SpecifiedLogisticsServiceCharge`` ✓ | ``LogisticsServiceCharge`` + nested ``AppliedTradeTax`` in ``schema/settlement.py``; ``TradeSettlement.logistics_service_charges`` field | 0..* | Landed. XSD position: between ``SpecifiedTradeAllowanceCharge`` and ``SpecifiedTradePaymentTerms`` (the original plan said "before SpecifiedTradeAllowanceCharge" — that was wrong). ``AppliedAmount`` still needs to flow into the charge-sum accumulators when §5.2 / §5.3 (``BR-FXEXT-CO-12/13/15`` and ``BR-FXEXT-{cat}-08``) land. |
...
| BG-X-45 / BG-X-46 ✓ | ``SpecifiedAdvancePayment`` + nested ``IncludedTradeTax`` | ``AdvancePayment`` (``PaidAmount`` BT-X-291, ``FormattedReceivedDateTime`` BT-X-292, list of ``AdvancePaymentTradeTax`` BG-X-46, optional ``AdvancePaymentReferencedDocument`` BG-X-85) in ``schema/settlement.py``; field ``TradeSettlement.advance_payments`` | 0..* | Landed. XSD position: last child of ``HeaderTradeSettlement`` (after ``ReceivableSpecifiedTradeAccountingAccount``). Required a fix to ``_date_inner_namespace`` so ``FormattedReceivedDateTime`` resolves its inner ``qdt:DateTimeString`` (now keys on the ``Formatted`` prefix). Exercised by ``EXTENDED_synth_settlement_parties.xml``. |
| — ✓ | ``SpecifiedTradePaymentTerms`` cardinality + per-term payee | ``terms`` is ``list[PaymentTerms]`` (landed in the prior PR); ``PartialPaymentAmount`` (BT-X-275, prior PR); per-term ``PayeeTradeParty`` (BG-X-77) field ``PaymentTerms.payee`` (this PR) | 0..* | Done. Per-term payee reuses the BG-10 ``PayeeTradeParty`` class. |
...
| — | ``SequenceNumeric`` | — | 0..1 | **Correction**: ``SequenceNumeric`` is **not** a line-item field — the XSD only puts it on ``TradeAllowanceChargeType`` (allowance/charge ordering, BT-X-265 / BT-X-268). Dropped from §4.5; tracked with the allowance/charge extras instead. |
| BG-X-1 ✓ | ``IncludedReferencedProduct`` | ``IncludedReferencedProduct`` on ``TradeProduct`` | 0..* | Done in the prior PR (``EXTENDED_zf24_Warenrechnung.xml``). |
| BG-X-84 ✓ | ``IndividualTradeProductInstance`` | ``IndividualTradeProductInstance`` (``BatchID`` BT-X-306 + ``SupplierAssignedSerialID`` BT-X-307) | 0..* | Done in the prior PR (``EXTENDED_zf24_Maschinen_Serial.xml``). |
| BG-X-90 ✓ | ``ItemSellerTradeParty`` | ``ItemSellerTradeParty`` in ``schema/party.py``; field ``LineTradeAgreement.item_seller`` | 0..1 | Landed. XSD position: on ``SpecifiedLineTradeAgreement`` after ``NetPriceProductTradePrice`` (**not** on ``TradeProduct`` as the original plan said). Exercised by ``EXTENDED_synth_product_line.xml``. |
| BT-X-532/533/535/536 ✓ | ``IndustryAssignedID``, ``ModelID``, ``BrandName``, ``ModelName`` | flat optional fields on ``TradeProduct`` (added prior PR) | 0..1 each | Now exercised by ``EXTENDED_synth_product_line.xml``. ``ClassName`` (BT-X-13) landed earlier on ``DesignatedProductClassification``; ``ValueMeasure`` is part of ``ApplicableProductCharacteristic`` (already modelled). |
| BG-X-7 / BG-X-10 ✓ | Line-level ``ShipToTradeParty`` / ``UltimateShipToTradeParty`` | fields ``LineTradeDelivery.ship_to`` / ``.ultimate_ship_to`` (reuse the header party classes) | 0..1 | Landed. Exercised by ``EXTENDED_synth_product_line.xml``. |
| BT-X-46/47/561 ✓ | ``ChargeFreeQuantity``, ``PackageQuantity``, ``PerPackageUnitQuantity`` | ``Quantity`` subclasses + fields on ``LineTradeDelivery`` | 0..1 each | Landed. Exercised by ``EXTENDED_synth_product_line.xml``. |
| — | Line-level ``SellerOrderReferencedDocument`` / ``ContractReferencedDocument`` / ``AdditionalReferencedDocument`` / ``UltimateCustomerOrderReferencedDocument`` / ``ApplicableTradeDeliveryTerms`` | — | 0..1/0..* | **Deferred**: per-line twins of structures already modelled at header level; low marginal value and no observed real-world use. Add when a sample needs them. |
