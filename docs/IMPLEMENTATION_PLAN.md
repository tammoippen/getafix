...
| 2 | `schema/accounting.py` — `MonetarySummation.line_total`  | Required field, but at MINIMUM the XSD does not include `LineTotalAmount`. Round-tripping a real MINIMUM sample crashes the writer.       | Make `line_total` `Decimal \| None` and gate it on `>= BASIC_WL`. Validator BR-12 is what enforces presence at `>= BASIC_WL`. | **fixed** (BR-12 still pending) |
| 3 | `schema/accounting.py` — `ApplicableTradeTax.exemption_reason_code` | Field metadata `tag="ExemptionReason"` (copy-paste from the previous field). Should be `"ExemptionReasonCode"`. Right now it shadows BT-120 and BT-121 is never written. | Fix the tag literal. | **fixed** |
| 4 | `schema/accounting.py` — `TradeAllowanceCharge.basis_amount` | Field metadata `tag="CalculationPercent"` (copy-paste from previous field). BT-93/BT-100 is never written. | Fix to `"BasisAmount"`. | **fixed** |
| 5 | `schema/party.py` — `BuyerTradeParty.tax_registrations` | Single `SpecifiedTaxRegistration`, but the XSD permits 0..2 (one VA, one FC). Same on `PayeeTradeParty`. | Change to `list[SpecifiedTaxRegistration] \| None`. | **fixed** for `BuyerTradeParty`; `PayeeTradeParty` doesn't model the field yet (separate gap) |
| 6 | `schema/accounting.py` — `MonetarySummation.tax_total`   | Single `TaxTotal`, but the XSD allows 0..2 (BT-110 invoice currency + BT-111 VAT accounting currency). Real samples with foreign-currency invoices cannot round-trip. | Change to `list[TaxTotal] \| None`. Add validator BR-53 (BT-6 set ⇒ second TaxTotal present). | **fixed** (BR-53 still pending — BT-6 not modelled yet) |
| 7 | `schema/element.py` — XML attribute parser                | `_parse_str` reads only the element body, throwing away any `currencyID` attribute on `udt:AmountType` elements. Round-tripping any real header-level amount loses the attribute. | Each amount-bearing dataclass carries a sibling ``currency: str \| None`` field; ``_children_xml`` stamps it onto every ``"amount": True`` element as ``currencyID``, and ``from_xml`` captures the first parsed ``currencyID`` back into ``currency``. | **fixed** |
...
5. **BR-AE/BR-E/BR-G/BR-IC/BR-AF/BR-AG/BR-O/BR-S/BR-Z (the `-2/3/4` rules)** — VAT-category ⇒ required parties matrix — **done**. Implemented per-category in `carthorse.rules.trade`; the BR-O family lives next to it (inverted predicate, including the BT-46 vs BT-48 spec quirk).
