...
| 7 | `schema/element.py` — XML attribute parser                | `_parse_str` reads only the element body, throwing away any `currencyID` attribute on `udt:AmountType` elements. Round-tripping any real header-level amount loses the attribute. | Capture leaf-element attributes into ``Element._xml_attrs`` on parse and replay them on render. ``_xml_attrs`` is excluded from ``__init__`` / ``repr`` / ``compare`` so the public API doesn't shift. | **fixed** |
...
5. **BR-AE/BR-E/BR-G/BR-IC/BR-AF/BR-AG/BR-O/BR-S/BR-Z (the `-2/3/4` rules)** — VAT-category ⇒ required parties matrix — **done**. Implemented data-driven in `Trade._validate_vat_category_required_parties`; the BR-O family lives next to it (inverted predicate, including the BT-46 vs BT-48 spec quirk).
